...
beta1 = (N_reg * Sxy - Sx * Sy) / (N_reg * Sxx - Sx * Sx)
...
for chunk in pd.read_csv(BIGFILE, sep=SEP, chunksize=CHUNK_SIZE):
    ...
    # target: whether any correct in this session
    df_p["y"] = (df_p["session_correct"] > 0).astype(int)

    rows_list.append(df_p[["f_L", "g_T", "h_l", "y"]])
...
prior_df = prior_df.dropna(subset=["f_L", "g_T", "h_l", "y"])
...
X_prior = prior_df[["f_L", "g_T", "h_l"]].values
X_prior = sm.add_constant(X_prior)  # adds intercept: [1, f_L, g_T, h_l]
y_prior = prior_df["y"].values
...
def nll_over_features(theta, user_set, max_rows=None):
    """
    Average NLL over rows in FEATURE_FILE whose user_id is in user_set.
    Uses streaming; if max_rows is set, stops after that many rows.
    """
    LL_sum = 0.0
    N_total = 0
